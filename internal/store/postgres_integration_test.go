...
// codeSeq is a process-wide monotonic counter that guarantees uniqueCode
// never repeats, even when called multiple times within the same
// microsecond (the timestamp's resolution).
var codeSeq atomic.Uint64

// uniqueCode returns a short code that's safe to insert without colliding
// with other codes generated by this test binary. The timestamp keeps the
// value human-readable; the atomic suffix guarantees uniqueness across
// rapid successive calls and parallel tests sharing the process.
func uniqueCode(t *testing.T) string {
	t.Helper()
	return fmt.Sprintf("t%s-%d", time.Now().UTC().Format("150405.000000"), codeSeq.Add(1))
}
